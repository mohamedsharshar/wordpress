...
class Custom_Layouts_Cpt {
	...
	public function change_cpt_label() {
		...
		if ( $supports_new_menu && 'valid' === apply_filters( 'product_neve_license_status', false ) ) {
			...
			if ( ! current_user_can( $capability ) ) {
				$capability = 'edit_custom_layouts';
				...
				add_menu_page(
					...
					'', // @phpstan-ignore-line - no need to add a callback as this points to the edit page.
					'dashicons-layout',
					99
				);
			}
	...
	public function add_role_caps() {
		...
		foreach ( $roles as $current_role ) {
			...
			$role->add_cap( 'delete_custom_layouts', true );
			$role->add_cap( 'delete_others_custom_layouts', true );
		}
	}
	 ...
	 */
	final public function register_custom_post_type() {
		...
		$labels = array(
			...
			'delete_item'   => esc_html__( 'Delete Custom Layout', 'neve-pro-addon' ),
		);
		...
		$args = array(
			...
			'show_in_rest'        => true,
			'supports'            => array( 'title', 'editor', 'elementor', 'delete' ),
			...
			'capabilities'        => array(
				...
				'delete_posts'        => 'delete_custom_layouts',
				'delete_others_posts' => 'delete_others_custom_layouts',
			),
		);
