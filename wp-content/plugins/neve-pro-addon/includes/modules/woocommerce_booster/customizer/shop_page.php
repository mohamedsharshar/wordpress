...
class Shop_Page extends Base_Customizer {
	...
	public function init() {
		...
		add_action( 'customize_register', array( $this, 'remove_woocommerce_layout_controls' ), 20 );
	...
	/**
	 * Remove the Products per row & Rows per page controls.
	 */
	public function remove_woocommerce_layout_controls() {
		$this->wpc->remove_control( 'woocommerce_catalog_columns' );
		$this->wpc->remove_control( 'woocommerce_catalog_rows' );
	}
	...
	private function add_page_layout_controls() {
		...
		$cols = get_option( 'woocommerce_catalog_columns', 3 );
	...
	private function add_card_content_controls() {
		...
		$components    = array(
			...
			'title'             => $content_align === 'inline' ? __( 'Title', 'neve-pro-addon' ) . ' + ' . __( 'Product Price', 'neve-pro-addon' ) : __( 'Title', 'neve-pro-addon' ),
