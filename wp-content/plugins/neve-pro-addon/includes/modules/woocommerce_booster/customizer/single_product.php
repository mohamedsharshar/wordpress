...
class Single_Product extends Base_Customizer {
	...
	public function add_controls() {
		...
		$this->add_upsell_accordion();
		$this->add_upsell_count();
	}
	...
	private function add_checkboxes() {
		$checkboxes = array(
			...
			'neve_enable_product_upsell_slider'     => array(
				'default'         => false,
				'priority'        => 52,
				'active_callback' => array( $this, 'hide_if_upsell_disabled' ),
				'label'           => __( 'Enable Upsell Products Slider', 'neve-pro-addon' ),
			),
	...
	/**
	 * Adds upsell products settings accordion
	 */
	private function add_upsell_accordion() {
		$this->add_control(
			new Control(
				'neve_upsell_products_heading',
				array(
					'sanitize_callback' => 'sanitize_text_field',
				),
				array(
					'label'            => esc_html__( 'Upsell Products', 'neve-pro-addon' ),
					'section'          => 'neve_single_product_layout',
					'priority'         => 50,
					'class'            => 'upsell-products-accordion',
					'accordion'        => true,
					'expanded'         => false,
					'controls_to_wrap' => 4,
				),
	...
	/**
	 * Add upsell products number control.
	 */
	private function add_upsell_count() {
		$this->add_control(
			new Control(
				'neve_single_product_upsell_count',
				array(
					'sanitize_callback' => 'absint',
				),
				array(
					'label'           => esc_html__( 'Number of Upsell Products', 'neve-pro-addon' ),
					'section'         => 'neve_single_product_layout',
					'default'         => 4,
					'type'            => 'number',
					'input_attrs'     => array(
		...
		$this->add_control(
			new Control(
				'neve_single_product_upsell_columns',
				array(
					'sanitize_callback' => 'absint',
				),
				array(
	...
	/**
	 * Active callback for when upsell is disabled.
	 *
	 * @return mixed|string
	 */
	public function hide_if_upsell_disabled() {
		return get_theme_mod( 'neve_enable_product_upsells', true );
	}
