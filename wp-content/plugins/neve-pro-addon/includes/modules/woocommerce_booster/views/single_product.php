...
class Single_Product extends Base_View {
	...
	public function run() {
		...
		add_filter( 'woocommerce_upsell_display_args', array( $this, 'upsell_products_args' ), 20 );
	...
	/**
	 * Change upsell products arguments.
	 *
	 * @param array $args query parameters for upsell products.
	 *
	 * @return mixed
	 */
	public function upsell_products_args( $args ) {
		$upsell_count = get_theme_mod( 'neve_single_product_upsell_count', 4 );
		$upsell_cols  = get_theme_mod( 'neve_single_product_upsell_columns', 4 );
		if ( empty( $upsell_count ) && empty( $upsell_cols ) ) {
			return $args;
		}

		if ( ! empty( $upsell_cols ) ) {
			$args['columns'] = absint( $upsell_cols );
		}

		if ( ! empty( $upsell_count ) ) {
			$args['posts_per_page'] = absint( $upsell_count ); //phpcs:ignore WordPress.WP.PostsPerPage.posts_per_page_posts_per_page
		}
