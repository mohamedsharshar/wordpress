...
class Dynamic_Style extends Generic_Style {
	...
	const MODS_SHOP_SINGLE_UPSELL_COLUMN  = 'neve_single_product_upsell_columns';
	...
	public function single_product_catalog_subscribers( $subscribers ) {
		...
		$subscribers[] = [
			'selectors' => '.products.upsells .products',
			'rules'     => [
				'--shopcoltemplate' => [
					Dynamic_Selector::META_DEFAULT     => 4,
					Dynamic_Selector::META_DEVICE_ONLY => 'desktop',
					Dynamic_Selector::META_KEY         => self::MODS_SHOP_SINGLE_UPSELL_COLUMN,
				],
			],
		];
		...
		$rules = [
			'--shopcoltemplate' => [
				...
				Dynamic_Selector::META_IS_RESPONSIVE => true,
				Dynamic_Selector::META_DEFAULT       => '{"desktop":' . absint( $cols ) . ',"tablet":2,"mobile":2}',
			],
		];
