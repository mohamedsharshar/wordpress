...
class Layout_CPT_Single extends Base_Customizer {
	...
	private function post_navigation() {
		...
		$this->add_control(
			new Control(
				'neve_' . $this->model->get_type() . '_nav_infinite_same_category',
				[
					'sanitize_callback' => 'neve_sanitize_checkbox',
					'default'           => true,
				],
				[
					...
					'priority'        => 365,
					'active_callback' => function () {
						return $this->model->is_custom_layout_enabled() && $this->element_is_enabled( 'post-navigation' ) && Mods::get( 'neve_' . $this->model->get_type() . '_nav_infinite', false );
					},
				]
			)
		);
