	...
	final public function register_dynamic_theme_mods() {
		...
		add_filter(
			...
			function ( $value ) {
				...
				if ( $this->model->is_archive_or_taxonomy_page() && $this->model->is_custom_layout_archive_enabled() ) {
					return $separator;
				}
		...
		add_filter(
			...
			function ( $value ) {
				...
				if ( $this->model->is_archive_or_taxonomy_page() ) {
					return $avatar_size;
				}
				if ( is_singular( $this->model->get_type() ) ) {
		...
		if ( $this->is_blog_pro_enabled ) {
			...
			add_filter(
				'theme_mod_neve_post_nav_infinite_same_category',
				function ( $value ) {
					if ( is_singular( $this->model->get_type() ) ) {
						return Mods::get( 'neve_' . $this->model->get_type() . '_nav_infinite_same_category', $value );
					}
