...
class Mega_Menu {
	...
	public function __construct() {
		...
		// Backend
		add_action( 'init', [ $this, 'setup_colors' ], 0 );
	 ...
	 * Setup the colors.
	 * 
	 * @hooked init 0 - needed to properly load translations.
	 */
	...
	public function load_backend_app() {
		...
		wp_set_script_translations( 'neve-mega-menu-app', 'neve-pro-addon' );

		// Enqueue the chunks files so that we can load the translation files.
		if ( isset( $dependencies['chunks'] ) ) {
			foreach ( $dependencies['chunks'] as $chunk_file ) {
		
				$chunk_handle = 'neve-pro-mega-menu-chunk-' . $chunk_file;
				wp_register_script( $chunk_handle, NEVE_PRO_URL . $relative_path . 'build/' . $chunk_file, [], $dependencies['version'], true );
				wp_enqueue_script( $chunk_handle );
				
				wp_set_script_translations( $chunk_handle, 'neve-pro-addon' );
			}
		}
	}
