...
class Page_Header extends Abstract_Builder {
	...
	const DISPLAY_OTHER_PAGE_TITLE = 'neve_pro_page_header_show_other_page_title';
	...
	public function render_on_neve_page_header() {
		...
		$is_search_page_enabled = get_theme_mod( 'neve_pro_page_header_search_result_enabled', false );
		if ( $is_search_page_enabled && is_search() ) {
			$will_render = true;
		}
	...
	/**
	 * Adds the controls for other pages.
	 *
	 * @param WP_Customize_Manager $wp_customize The WP Customize object.
	 *
	 * @return WP_Customize_Manager
	 */
	private function add_other_pages_controls( WP_Customize_Manager $wp_customize ) {
		// Display archive title setting
		$wp_customize->add_setting(
			self::DISPLAY_OTHER_PAGE_TITLE,
			[
				'transport'         => 'refresh',
				'sanitize_callback' => 'sanitize_text_field',
				'default'           => esc_html__( 'Show on Other Pages', 'neve-pro-addon' ),
			]
		);
		...
		$heading = new Heading(
			...
			[
				'type'             => 'neve_customizer_heading',
				'section'          => self::GLOBAL_SETTINGS_SECTION,
				'label'            => esc_html__( 'Show on Other Pages', 'neve-pro-addon' ),
				'description'      => esc_html__( 'Select Pages', 'neve-pro-addon' ),
				'accordion'        => true,
				'expanded'         => true,
				'controls_to_wrap' => 1,
				'class'            => self::DISPLAY_OTHER_PAGE_TITLE . '-accordion',
			]
		);
		$wp_customize->add_control( $heading );
		$this->customizer_style .= '.' . self::DISPLAY_OTHER_PAGE_TITLE . '-accordion.expanded {margin-bottom: 0;}';

		$wp_customize->add_setting(
			'neve_pro_page_header_search_result_enabled',
			[
				'transport'         => 'refresh',
				'sanitize_callback' => 'neve_sanitize_checkbox',
				'default'           => '',
		...
		$wp_customize->add_control(
			...
			[
				'type'    => 'neve_toggle_control',
				'label'   => __( 'Search Page', 'neve-pro-addon' ),
				'section' => self::GLOBAL_SETTINGS_SECTION,
			]
		);
	...
	public function customize_register( WP_Customize_Manager $wp_customize ) {
		...
		// Add other page toggles.
		$wp_customize = $this->add_other_pages_controls( $wp_customize );
