...
class Loader {
	...
	protected $hooks_map = array(
		...
		'neve_do_maintenance'                  => array(
			'hooks_to_deactivate' => array( 'neve_do_maintenance' ),
			'posts_map_key'       => 'maintenance',
		),
		'neve_do_coming_soon'                  => array(
			'hooks_to_deactivate' => array( 'neve_do_coming_soon' ),
			'posts_map_key'       => 'coming_soon',
		),
	);
			...
			foreach ( $post_map as $layout => $posts ) {
				switch ( $layout ) {
					...
					case 'server_error':
						...
						break;
					case 'maintenance':
						add_action( 'neve_do_maintenance', [ $this, 'render_first_markup' ], 1 );
						break;
					case 'coming_soon':
						add_action( 'neve_do_coming_soon', [ $this, 'render_first_markup' ], 1 );
						break;
					case 'individual':
