...
class Setting {
	...
	private function toggle_all_subsettings( $main_module, $setting ) {
		...
		if ( empty( $all_subsettings ) ) {
			/* translators: %1$s: Module name passed via wp cli. */
			$message = sprintf( esc_html__( 'Module %1$s does not contain subsettings.', 'neve-pro-addon' ), $main_module ) . Messages::get_wrong_command_text();
			\WP_CLI::error( $message, true );
		}
